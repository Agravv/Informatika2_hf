<?php
// todo egy projekthez egy project_lead tartozzon, akit az admin nevez ki
...
if (mysqli_num_rows($result_not_started) > 0) {
    ...
    while ($row = mysqli_fetch_array($result_not_started)) {
        ...
        $query_not_started_with_leader = "SELECT project.project_id,project.title,project.description,project.due_date,project.status 
                                            FROM project
                                            LEFT JOIN user_has_project ON user_has_project.project_project_id = project.project_id
        ...
        if (mysqli_num_rows($result_not_started_with_leader) > 0) {
            ...
            echo '<td><a href="edit_projects_admin.php?status=not_started&has_leader=false&project_id=' . $row['project_id'] . '"> <i
    ...
    while ($row = mysqli_fetch_array($result_not_started_with_leader)) {
        ...
        if (mysqli_num_rows($result_employee) > 0) {
            ...
            echo '  </select></td><td>
                        ...
                        <td><a href="edit_projects_admin.php?status=not_started&has_leader=true&project_id=' . $row['project_id'] . '&leader_id=' . $row['user_id'] . '"> <i
                        class="fa-solid fa-pen-to-square" style="color: ' . $color_code . ';"></i> </a></td>
                        <td><a href="delete_projects.php?status=in_progress&project_id=' . $row['project_id'] . '"> <i class="fa-solid fa-trash"
                        style="color: ' . $color_code . ';"></i> </a></td>
                        </tr>';
...
//                                     ORDER BY due_date;";
// $result_not_started_with_leader = mysqli_query($link, $query_not_started_with_leader);
// if (mysqli_num_rows($result_not_started) > 0 && mysqli_num_rows($result_not_started_with_leader) == 1) {
//     echo '<div class="table-container"><table class="user-table table-hover table">
//                 <caption><h4>Hozzárendelésre váró projektek</h4></caption>
//                 <th>ID</th>
//                 <th>Projekt név</th>
...
//                     <td>' . $row['due_date'] . '</td>
//                     ';
//         $query_users = "SELECT user_id, username
//                         FROM user
//                         WHERE access_level = 'employee';";
...
//                 echo '<option value="' . $row_user['user_id'] . '_' . $row['project_id'] . '">' . $row_user['username'] . '</option>';
//             }
//             echo '  </select></td><td>
//                         <input type="submit" name="submit_assign_to_leader" value="Kiosztás" class="button" style="font-size:1rem">
//                         </td>
//                         </form>
//                         <td><a href="edit_projects_admin.php?status=not_started&has_leader=true&project_id=' . $row['project_id'] . '"> <i
//                         class="fa-solid fa-pen-to-square" style="color: ' . $color_code . ';"></i> </a></td>
//                         <td><a href="delete_projects.php?status=in_progress&project_id=' . $row['project_id'] . '"> <i class="fa-solid fa-trash"
//                         style="color: ' . $color_code . ';"></i> </a></td>
//                         </tr>';
...
if (mysqli_num_rows($result_in_progress_employee) > 0) {
    ...
    while ($row = mysqli_fetch_array($result_in_progress_employee)) {
        ...
        echo '<td><a href="edit_projects_admin.php?status=in_progress&has_leader=true&project_id=' . $row['project_id'] . '&leader_id=' . $row_lead['user_id'] . '"> <i
                class="fa-solid fa-pen-to-square" style="color: ' . $color_code . ';"></i> </a></td>
                <td><a href="delete_projects.php?status=in_progress&project_id=' . $row['project_id'] . '"> <i class="fa-solid fa-trash"
                style="color: ' . $color_code . ';"></i> </a></td>';
    }
...
if (mysqli_num_rows($result_finished) > 0) {
    ...
    while ($row = mysqli_fetch_array($result_finished)) {
        ...
        echo '<td><a href="edit_projects_admin.php?status=finished&has_leader=true&project_id=' . $row['project_id'] . '&leader_id=' . $row_leader['user_id'] . '"> <i
                class="fa-solid fa-pen-to-square" style="color: ' . $color_code . ';"></i> </a></td>
                        <td><a href="delete_projects.php?status=finished&project_id=' . $row['project_id'] . '"> <i class="fa-solid fa-trash"
                        style="color: ' . $color_code . ';"></i> </a></td>';
